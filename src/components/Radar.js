...
const Radar = ({ isActive, matchingState, distance, signalBars }) => {
  ...
  useEffect(() => {
    if (isActive) {
      const pulseDuration =
        matchingState === 'scanning' ? 2000 :
        matchingState === 'matching' ? 1200 :
        matchingState === 'pinpointing' ? 600 : 3000;
  ...
  const getStatusMessage = () => {
    switch (matchingState) {
      ...
      case 'matching':
        return theme.lcd ? 'Syncing...' : 'Establishing connection...';
      case 'pinpointing':
        return theme.lcd ? 'Signal locked...' : 'Pinpointing match...';
      case 'match_found':
        return theme.lcd ? 'Match found!' : 'Match found!';
      default:
  ...
  return (
    <View style={styles.container}>
      ...
      {matchingState !== 'pinpointing' && <Animated.View style={[styles.pulse, { delay: 1000 }]} />}
