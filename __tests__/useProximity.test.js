...
describe('useProximity', () => {
  ...
  it('should transition through states to "match_found" after delays', async () => {
    ...
    await act(async () => {
      jest.advanceTimersByTime(5000); // Trigger searching -> signal_found
    });
    expect(result.current.matchingState).toBe('signal_found');

    await act(async () => {
      jest.advanceTimersByTime(4000); // Trigger signal_found -> connecting
    });
    expect(result.current.matchingState).toBe('connecting');

    await act(async () => {
      jest.advanceTimersByTime(3000); // Trigger connecting -> match_found
    });
  ...
  it('should transition to "bridge" when match is accepted', async () => {
    ...
    await act(async () => {
      jest.advanceTimersByTime(15000); // Advance through all states
    });
